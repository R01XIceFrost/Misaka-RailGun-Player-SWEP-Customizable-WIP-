...
if CLIENT then
    ...
    killicon.Add( "weapon_dmisakarailgun", "d_misakarailgun_mats/finger_killicon", Color(220, 15, 245, 225) )
...
SWEP.Purpose = "Blast your enemies away\n*The old version of Misaka RailGun has been moved to Misaka Network (Misaka #10030)"
...
SWEP.ViewModel = Model( "models/weapons/c_coin.mdl" )
SWEP.WorldModel = Model( "models/weapons/w_coin.mdl" )
SWEP.UseHands = true
...
local function pitch_yaw_to_vector( pitch, yaw )

    local pitch_rad = math.rad(pitch)
    local yaw_rad = math.rad(yaw)

    local x = math.cos( yaw_rad ) * math.cos(pitch_rad)
    local y = math.sin(yaw_rad) * math.cos(pitch_rad)
    local z = math.sin(pitch_rad)

    return Vector(x,y,-z)

end

local function dir_to_euler(dir)

    local pitch = math.atan2(-dir.z, math.sqrt(dir.x * dir.x + dir.y * dir.y) ) 
...
local function rotate_dir(forward, right, up, pitch, yaw)

    local pa = math.rad(pitch)
    local ya = math.rad(yaw) 

    local cp = math.cos(pa)
    local sp = math.sin(pa)

    local cy = math.cos(ya)
    local sy = math.sin(ya)

    local pitchX = cp * forward.x + sp * up.x
    local pitchY = cp * forward.y + sp * up.y
    local pitchZ = cp * forward.z + sp * up.z

    local finalX = cy * pitchX + sy * right.x
    local finalY = cy * pitchY + sy * right.y
    local finalZ = cy * pitchZ + sy * right.z

    return Vector(finalX, finalY, finalZ):GetNormalized()
end

local function rodrigues_rotation(v, axis, rotate_amount)
    local cosA = math.cos(rotate_amount)
    local sinA = math.sin(rotate_amount)
    
    local dot = axis.x * v.x + axis.y * v.y + axis.z * v.z

    local cross = axis:Cross(v)
    
    local x = v.x * cosA + cross.x * sinA + axis.x * dot * (1 - cosA)
    local y = v.y * cosA + cross.y * sinA + axis.y * dot * (1 - cosA)
    local z = v.z * cosA + cross.z * sinA + axis.z * dot * (1 - cosA)
    
    return Vector(x,y,z)

end

local function rotate_rodrigue(dir, pitch, yaw)

	local right = get_right_vector(dir)
	local up = get_vector_up(dir)

	local rotated_pitch = rodrigues_rotation(dir, right, math.rad(pitch) )

	local rotated_final = rodrigues_rotation(rotated_pitch, up, math.rad(yaw) )
...
function SWEP:Deploy()
    ...
    local cd = CurTime() + 0.6
    self.RailGunBlast_Cooldown = cd
    self.RedeployVMCD = 0
...
SWEP.Beam_CoinTossed = false
SWEP.DelayBeforeCoinToss = 0
SWEP.CoinTossHold = 0

SWEP.Beam_Charging_Sound = nil
SWEP.Beam_Blasting_Sound = nil
SWEP.Beam_Charged_Sound = nil
SWEP.Coin_Toss_Sound = nil

SWEP.Current_Beam_Dist = 9000

SWEP.BeamDamage_CD = 0

SWEP.BeamChargingCD = 0

SWEP.RedeployVMCD = 0

SWEP.HandThundersCD = 0
...
function SWEP:ProcessBeam()
    ...
    self.Beam_Charging_Sound = ( self.Beam_Charging_Sound == nil ) && CreateSound( self, "weapons/physcannon/hold_loop.wav" ) or self.Beam_Charging_Sound
    self.Beam_Blasting_Sound = ( self.Beam_Blasting_Sound == nil ) && CreateSound( self, "npc/stalker/laser_burn.wav" ) or self.Beam_Blasting_Sound
    self.Beam_Charged_Sound = ( self.Beam_Charged_Sound == nil ) && CreateSound( self, "items/battery_pickup.wav" ) or self.Beam_Charged_Sound
    self.Coin_Toss_Sound = ( self.Coin_Toss_Sound == nil ) && CreateSound( self, "weapons/coin/tales_of_berseri_eizen_flip_coin.mp3" ) or self.Coin_Toss_Sound
    ...
    local holding_left = self.Owner:KeyDown(IN_ATTACK)
    ...
    --This part had to be coded improvisedly post-code, since I first coded the weapon to shoot the laser instantly after releasing left click, then was requested to add this cooldown for the coin-toss animation
    local hold_coin = ( self.CoinTossHold >= CurTime() ) && true or (!holding_left)

    if ( self.DelayBeforeCoinToss >= CurTime() && hold_coin ) then

        local prev_ = self.CoinTossHold

        if ( prev_ < CurTime() ) then
            self:SendWeaponAnim(ACT_VM_PRIMARYATTACK)
            self.Coin_Toss_Sound:PlayEx(1,100)
        end

        self.CoinTossHold = CurTime() + 1.1
    ...
    --emit blast
    if ( self:GetRailGunCharge() > 0 && ( keep_blasting or hold_coin ) ) then
        ...
        if ( self.RailGun_RingEmited == false ) then
            ...
            local ring_string = GetConVarString("d_misakarailgun_visuals_ring_texture")
            local ring_texture = (ring_string != nil && ring_string != "") && ring_string or "d_misakarailgun_mats/beam_material"
         
            D_MisakaGun_AddRing_Sv( d_pos, self.Owner:EyeAngles(), ring_texture, max_ring_width, max_ring_radius, 32, max_jag, ring_color, 1.1, 95, 1.2 )
            D_MisakaGun_AddRing_Sv( d_pos_Seconds, self.Owner:EyeAngles(), ring_texture, max_ring_width, max_ring_radius, 32, max_jag, ring_color, 1.1, 95, 1.2 )
        ...
        stop_sound(self.Beam_Charged_Sound:IsPlaying(),self.Beam_Charged_Sound)
        stop_sound(self.Coin_Toss_Sound:IsPlaying(),self.Coin_Toss_Sound)
        stop_sound( charge_snd:IsPlaying(), charge_snd )
    ...
    self.RedeployVMCD = ( self:GetEmittingBeam() ) && (CurTime() + 0.15) or self.RedeployVMCD
    ...
    if ( self.RedeployVMCD > 0 && self.RedeployVMCD < CurTime() ) then
        self:SendWeaponAnim(ACT_VM_DRAW)
        self.RedeployVMCD = 0
    end
    ...
    if ( self.RailGunBlast_Cooldown < CurTime() && holding_left && self:GetEmittingBeam() == false && hold_coin == false ) then

        self.Current_Beam_Dist = 0
        self.Beam_CoinTossed = false
        self.DelayBeforeCoinToss = CurTime() + 0.95
        self.BeamChargingCD = CurTime() + 0.1
        self.CoinTossHold = 0
        ...
        if ( self:GetRailGunCharge() >= 100 && charging_playing ) then
            ...
            self.Beam_Charging_Sound:Stop()
            self.Beam_Charged_Sound:PlayEx(1,100)
...
function SWEP:SendPlayerDeployAnim()

    if ( !IsValid(self.Owner) or self.Owner:IsPlayer() == false ) then
        return
    end

    net.Start("D_MisakaRailgun_SendDeployAnim")
...
function SWEP:ViewModelDrawn(vm)
    ...
    if ( self:GetRailGunCharge() > 0 ) then
        ...
        if ( self.HandThundersCD < CurTime() ) then

            local mul_ = math.Clamp(self:GetRailGunCharge()/100, 0, 1)
            local hand_cd = scale_multiplication(mul_,0.18,0.045)

            local forw = self.Owner:GetAimVector()
            local right = -get_right_vector(forw)
            local up = get_vector_up(forw)

            local start_pos = vm:GetPos() + forw * 25 + right * 8 + up * -10

            local random_dir = rotate_rodrigue(forw, 90, math.random(0,360))
            local end_pos = start_pos + random_dir * math.random(6,12) 
            ...
            for i = 1, 2 do
                ...
                random_dir = rotate_rodrigue(forw, math.random(0,360), 90)
                end_pos = start_pos + random_dir * math.random(6,12)

                local tbl = { Start = start_pos, HitPos = end_pos, Curves = math.random(3,5), Width = 2, Jagginess = math.random(3,6), Fade_In_Time = 0.1, Fade_Out_Time = ( 0.1 + 0.1) , time_started = CurTime(), Has_Faded_In = false, Color = Color(255,255,255,255), Float_Alpha = 255, Starting_Alpha = 255, Texture = texture_shock, Mat_ = nil, thunder_paths = nil, current_max_thunder = 1, Thunder_Formation_Time = 0.4 }

                D_MisakaGun_AddJaggedBeam_CL(tbl)

                

            end
                
            self.HandThundersCD = CurTime() + hand_cd
...
function SWEP:DrawWorldModel()
    ...
    if ( self.Owner != nil && IsValid(self.Owner) && self.Owner:IsPlayer() && self:GetRailGunCharge() > 0 ) then

        local b = self.Owner:LookupBone("ValveBiped.Bip01_R_Hand")
        ...
        if ( self.HandThundersCD < CurTime() ) then
            ...
            local start_pos = m:GetTranslation() + forw * 5 + get_right_vector(forw) * 2

            local random_dir = pitch_yaw_to_vector( math.random(0,360), 0 )
            local end_pos = start_pos + random_dir * math.random(8,16)
            ...
            for i = 1, 2 do
                ...
                random_dir = pitch_yaw_to_vector( math.random(0,360), 0 )
                end_pos = start_pos + random_dir * math.random(8,16)
...
if ( SERVER ) then
    ...
    util.AddNetworkString("D_MisakaRailgun_SendDeployAnim")
...
if ( CLIENT ) then
    ...
    net.Receive("D_MisakaRailgun_SendDeployAnim", function()
        ...
        wep:SendWeaponAnim(ACT_VM_DEPLOY)
...
if ( CLIENT ) then
    ...
    local default_jag_mat = Material("trails/electric")
